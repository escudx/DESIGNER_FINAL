...
def _animate_fade_in(win: tk.Misc, *, duration: int = 140, steps: int = 6) -> None:
    """Executa uma animação rápida de fade-in ao abrir uma janela."""
    if getattr(win, "_fade_in_applied", False):
        return
    setattr(win, "_fade_in_applied", True)
    ...
    interval = max(10, duration // max(1, steps))

    def step(i: int = 0) -> None:
        value = min(1.0, (i + 1) / max(1, steps))
        try:
...
class App(ctk.CTk):
    ...
    def _on_body_viewport_resize(self):
    ...
    def _resize_header(self):
        total_w = self._total_table_width(); viewport_w = max(1, self.body_canvas.winfo_width())
        inner_w = max(total_w, viewport_w)
        self.header_canvas.configure(scrollregion=(0,0,total_w,self.header_h), width=viewport_w, height=self.header_h)
    ...
    def _resize_rows(self):
        ...
        self.body_canvas.configure(scrollregion=(0,0,total_w,height))
    ...
    def _on_change_type(self, f: Field, new_type: str):
        ...
        if new_type == "Objeto":
            if f.origin_task or f.origin_field:
                messagebox.showwarning("Objeto", "Um campo do tipo Objeto não pode ter origem."); self._refresh_rows(); return
            t = self._get_task()
            already = any(x.ftype == "Objeto" and x.id != f.id for x in t.fields)
            if already:
                messagebox.showwarning("Objeto", "Só é permitido 1 Objeto por tarefa."); self._refresh_rows(); return
            if not self.project.object_type:
                ok = self.open_flow_object_type_dialog()
                if not ok:
                    self._refresh_rows(); return
            self._push_undo()
            f.ftype = "Objeto"
        ...
        if new_type not in LIST_FIELD_TYPES:
            if new_type != "Informativo":
                f.options = ""
    ...
    def open_origin_picker(self, f: Field):
        ...
        def ok():
            ...
            self._rebuild_metadata_cache()
            win.destroy(); self._refresh_rows()
    ...
    def open_cond_builder(self, f: Field):
        ...
        ctk.CTkButton(btn_row, text="Fechar", width=140, command=lambda: (win.destroy(), self._refresh_rows())).pack(side="left")
    ...
    def _center_toplevel(
        ...
        *,
        ...
        if transient and win is not self:
            win.transient(self)
        try:
            win.configure(fg_color=DARK_BG2)
        except Exception:
            pass

        # Mantém a janela invisível enquanto aguardamos o cálculo de layout
        # para evitar o flicker inicial em monitores múltiplos.
        try:
            win.attributes("-alpha", 0.0)
            setattr(win, "_centered_hidden_alpha", True)
        except Exception:
            pass
